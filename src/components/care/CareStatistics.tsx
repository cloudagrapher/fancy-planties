    ...
    <div className="grid grid-cols-2 lg:grid-cols-4 gap-2 sm:gap-4">
      ...
      <div className="rounded-2xl shadow-sm border border-slate-200/70 bg-emerald-50/70 backdrop-blur p-3 sm:p-6">
        <div className="flex items-center justify-between">
          <div>
            <p className="text-xs sm:text-sm font-medium text-emerald-700">Active Plants</p>
            <p className="text-xl sm:text-2xl font-bold text-emerald-600">{statistics.totalActivePlants}</p>
          </div>
      ...
      <div className="rounded-2xl shadow-sm border border-slate-200/70 bg-amber-50/70 backdrop-blur p-3 sm:p-6">
        <div className="flex items-center justify-between">
          <div>
            <p className="text-xs sm:text-sm font-medium text-amber-700">Care Streak</p>
            <p className="text-xl sm:text-2xl font-bold text-amber-600">{statistics.careStreakDays}</p>
      ...
      <div className="rounded-2xl shadow-sm border border-slate-200/70 bg-sky-50/70 backdrop-blur p-3 sm:p-6">
        <div className="flex items-center justify-between">
          <div>
            <p className="text-xs sm:text-sm font-medium text-sky-700">This Week</p>
            <p className="text-xl sm:text-2xl font-bold text-sky-600">{statistics.totalCareEventsThisWeek}</p>
      ...
      <div className="col-span-2 lg:col-span-4 rounded-2xl shadow-sm border border-slate-200/70 bg-gradient-to-r from-primary-50/70 to-secondary-50/70 backdrop-blur p-4 sm:p-6">
        <h3 className="text-sm font-semibold text-gray-900 mb-2 sm:mb-3">Care Alerts Summary</h3>
        <div className="grid grid-cols-3 gap-2 sm:gap-4">
          <div className="text-center">
            <div className={`text-xl sm:text-2xl font-bold ${statistics.overdueCount > 0 ? 'text-red-600' : 'text-gray-400'}`}>
              {statistics.overdueCount}
            </div>
            <div className="text-xs text-gray-600">Overdue</div>
          </div>
          <div className="text-center">
            <div className={`text-xl sm:text-2xl font-bold ${statistics.dueTodayCount > 0 ? 'text-amber-600' : 'text-gray-400'}`}>
              {statistics.dueTodayCount}
            </div>
            <div className="text-xs text-gray-600">Due Today</div>
          </div>
          <div className="text-center">
            <div className={`text-xl sm:text-2xl font-bold ${statistics.dueSoonCount > 0 ? 'text-yellow-600' : 'text-gray-400'}`}>
