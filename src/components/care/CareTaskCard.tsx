...
export default function CareTaskCard({ plant, onQuickCare, showUrgency = false }: CareTaskCardProps) {
  ...
  // Shared action button component to avoid duplication
  const CareActionButton = ({ careType, bgColor, hoverColor, icon, label }: {
    careType: string;
    bgColor: string;
    hoverColor: string;
    icon: string;
    ...
    <button
      ...
      aria-label={`Quick ${label.toLowerCase()} ${plant.displayName}`}
    >
      {isLoading ? (
    ...
    <div className={`rounded-lg border p-3 sm:p-4 ${statusInfo.color} transition-all hover:shadow-md w-full max-w-full overflow-hidden`}>
      ...
      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3">
        ...
        <div className="flex gap-2 sm:ml-4 sm:flex-shrink-0">
          <CareActionButton careType="fertilizer" bgColor="bg-green-600" hoverColor="hover:bg-green-700" icon="🌱" label="Fertilize" />
          <CareActionButton careType="water" bgColor="bg-blue-600" hoverColor="hover:bg-blue-700" icon="💧" label="Water" />
          <CareActionButton careType="inspect" bgColor="bg-indigo-600" hoverColor="hover:bg-indigo-700" icon="🔍" label="Inspect" />
        </div>
      </div>
