...
import type { NewCareHistory, CareHistory } from '@/lib/db/schema';

// Counter for unique test data
let careRecordCounter = 0;

// Care types and related data for realistic test records
const CARE_TYPES = ['fertilizer', 'water', 'repot', 'prune', 'inspect', 'other'] as const;
 ...
 */
export const createTestCareRecord = (overrides: Partial<NewCareHistory> = {}): NewCareHistory => {
  careRecordCounter++;
...
export const createTestCareHistory = (
  ...
  return records.sort((a, b) => new Date(b.careDate).getTime() - new Date(a.careDate).getTime());
};
...
export const createRealisticCareSchedule = (
  ...
  while (currentDate <= endDate) {
    ...
    if (Math.random() > 0.1) { // 90% chance of watering each week
      records.push(createTestWateringRecord(plantInstanceId, userId, {
    ...
    if ((month >= 2 && month <= 8) && currentDate.getDate() <= 7) { // March-September, first week
      records.push(createTestFertilizerRecord(plantInstanceId, userId, {
        careDate: new Date(currentDate),
      }));
    }
    ...
    if (currentDate.getDate() === 1 && month % 3 === 0) {
      records.push(createTestInspectionRecord(plantInstanceId, userId, {
        careDate: new Date(currentDate),
      }));
    }
    ...
    if (Math.random() > 0.95) { // 5% chance each week
      records.push(createTestPruningRecord(plantInstanceId, userId, {
        careDate: new Date(currentDate),
      }));
    }
    ...
    if (month === 3 && currentDate.getDate() <= 7 && Math.random() > 0.7) { // April, 30% chance
      records.push(createTestRepottingRecord(plantInstanceId, userId, {
        careDate: new Date(currentDate),
      }));
    }
...
export const createTestCareStatistics = (careRecords: NewCareHistory[]) => {
  ...
  if (careRecords.length > 0) {
    ...
    const daysDiff = (new Date(newestRecord.careDate).getTime() - new Date(oldestRecord.careDate).getTime()) / (1000 * 60 * 60 * 24);
    const weeksDiff = daysDiff / 7;
    stats.averageCaresPerWeek = weeksDiff > 0 ? parseFloat((careRecords.length / weeksDiff).toFixed(1)) : 0;
...
export const createCareTrackingTestData = (plantInstanceId = 1, userId = 1) => {
  ...
  const fullCareHistory = createRealisticCareSchedule(plantInstanceId, userId, 6);
